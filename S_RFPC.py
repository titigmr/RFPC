...
def inner_outer(data, begin, end, comp=False, name='Latent', r2=0, printing=False): 
    """
    ...
    new = pd.concat([block,comp_rfpc], axis=1)
    
    out = np.array(pd.concat([weight,loading, comm, redond], axis=1))
    out_outer = pd.DataFrame(out, columns=["weight","loading", "communality","redundancy"], index=f_pca.col_labels)
...
def variable_latente(data,blocs,vl):
    """
    ...
    for enum,bloc in enumerate(blocs):
        composante = inner_outer(data,bloc[0], bloc[1],comp=True,name=str(vl[enum]))
...
def calcul_r2_inner(all_reg, vl_data):
    """
...
def RFPC_estimate(blocs,data,r2_all,vl):
    """
    ...
    for enum, bloc in enumerate(blocs) :
        out_outer, out_inner = inner_outer(data,bloc[0],bloc[1], 
...
def RFPC(data,blocs,vl,path):
    ...
    vl_data = variable_latente(data,blocs,vl)
    all_reg = reg(path)
    r2_all, list_resul_inner = calcul_r2_inner(all_reg, vl_data)
    final_inner, final_outer = RFPC_estimate(blocs,data,r2_all,vl)
    
    R2_mean = final_inner.R2[final_inner.R2 != 0].mean()
    AVE_mean = final_inner.AVE.mean()
    GOF = np.sqrt(R2_mean*AVE_mean)

    
    return final_inner, final_outer, list_resul_inner, GOF
